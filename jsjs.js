// (function() {
	...
	// init jsjs, this is actually a selector
	// returns a jsjs object containing selected nodelist
	// $('.selector')
	// $('.selector', '.context')
	// $('.selector', context)
	// TODO: $('<div>') for creating new dom
	var JSJS = function(selector, context) {
		// simply pass params to query()
		return this.merge(query(selector, context));
	}

	// add length for jsjs object
	JSJS.prototype.length = 0;
	...
	// util: iterate the jsjs object
	JSJS.prototype.forEach = function(fn) {
		var i = 0;
		while( (i < this.length) && fn(this[i++]) ) {}
	}

	// util: merge an array or object into current jsjs object
	JSJS.prototype.merge = function(another) {
	...
	JSJS.prototype.on = function(event, selector, fn) {
	...
	// .off('click')
	// .off('click', fn)
	JSJS.prototype.off = function(event, fn) {
		// .off('click', function() {})
	...
	// .data('key')
	// .data('key', val)
	JSJS.prototype.data = function(key, val, /*internal*/private) {
	...
	window.$ = function(selector, context) {
		return new JSJS(selector, context);
	}
